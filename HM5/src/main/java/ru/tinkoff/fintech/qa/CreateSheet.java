...
public class CreateSheet {
    ...
    private static void pushDB(List<MainPojo> data){
        Connection connection;
        try {
            connection = DriverManager.getConnection(url, user, password);
            Statement stmt = connection.createStatement();
            for (int i = 0; i < data.size(); ++i) {
                String query = "INSERT INTO users_table(name, surname, patronymic, age, sex, dob, itn, postcode, country, city, street, house, apartment, region) VALUES('" + data.get(i).getName() + "','" + data.get(i).getSurname() + "','" + data.get(i).getPatronymic()+ "','" + String.valueOf(data.get(i).getAge()) + "','"+ data.get(i).getGender()+"','" + data.get(i).getDob().get(Calendar.YEAR) + "-" + (data.get(i).getDob().get(Calendar.MONTH) + 1) + "-" + data.get(i).getDob().get(Calendar.DAY_OF_MONTH) + "','" + String.valueOf(data.get(i).getItn()) + "','"+ String.valueOf(data.get(i).getPostcode()) + "','" + data.get(i).getCountry() + "','" + data.get(i).getCity() + "','" +  data.get(i).getStreet() + "','" + String.valueOf(data.get(i).getHouse()) + "','" + String.valueOf(data.get(i).getApartment()) + "','" + data.get(i).getRegion()+ "');";
                stmt.execute(query);
            }
            connection.close();
    ...
    private static void fromDB(List<MainPojo> data){
        Connection connection;
        ...
        try {
            ...
            for (int i = 0; rs.next(); ++i) {
                data.add(new MainPojo());
                data.get(i).setName(rs.getString("name"));
                data.get(i).setSurname(rs.getString("surname"));
                data.get(i).setPatronymic(rs.getString("patronymic"));
                data.get(i).setGender(rs.getString("gender"));
                data.get(i).setPostcode(rs.getInt("postcode"));
                data.get(i).setCity(rs.getString("city"));
                data.get(i).setStreet(rs.getString("street"));
                data.get(i).setHouse(rs.getInt("house"));
                data.get(i).setApartment(rs.getInt("apartment"));
                data.get(i).setCountry(rs.getString("country"));
                data.get(i).setRegion(rs.getString("region"));
                data.get(i).setItn(Long.parseLong(rs.getString("itn")));
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                Calendar cal = Calendar.getInstance();
                cal.setTime(sdf.parse(rs.getString("dob")));
                data.get(i).setDob(cal);
                data.get(i).setAge(rs.getInt("age"));
            }
